...
- Each artifact's idempotence id is `messageType_messageIdempotenceId_system_schema_referenceId[_version]`
  (a discriminator making each artifact independently idempotent); `MessageArchiveService` fails fast if
