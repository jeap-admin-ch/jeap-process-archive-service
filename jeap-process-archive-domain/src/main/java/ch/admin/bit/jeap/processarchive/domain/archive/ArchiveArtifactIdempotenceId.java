 ...
 * if present - version), keeping the ID unique per artifact. Hashing bounds the length of the ID to the
 * message type name plus 65 characters, while the message type prefix keeps the ID attributable in logs.
 * The ID is derived deterministically so retries stay idempotent.
 */
public final class ArchiveArtifactIdempotenceId {
    ...
    public static String create(String messageTypeName, String messageIdempotenceId,
        ...
        String hashInput = messageIdempotenceId
                + "_" + system
                + "_" + schema
                + "_" + referenceId
